...
public class EditorMediaActivity extends Activity {
    ...
    private void initData() {
        ...
        mInputParam = new AlivcEditInputParam.Builder().setFrameRate(mFrameRate).setGop(mGop).setRatio(mRatio).setVideoQuality(mVideoQuality).setResolutionMode(mResolutionMode).setVideoCodec(mVideoCodec).setCrf(mCrf).setScaleRate(mScaleRate).setScaleMode(mScaleMode).setHasTailAnimation(mHasTailAnimation).setCanReplaceMusic(canReplaceMusic).addMediaInfos(mediaInfos).setDeNoise(hasDeNoise).setHasWaterMark(hasWaterMark).build();
    }
    ...
    private void init() {
        ...
        mMutiMediaView.setOnSelectMediaChangeListener(new MutiMediaView.OnSelectMediaChangeListener() {
            ...
            public void onClick(MediaInfo info, int position) {
                ...
                AlivcCropInputParam cropInputParam = new AlivcCropInputParam.Builder().setRatioMode(mInputParam.getRatio()).setResolutionMode(mInputParam.getResolutionMode()).setCropMode(mInputParam.getScaleMode()).setFrameRate(mInputParam.getFrameRate()).setGop(mInputParam.getGop()).setQuality(mInputParam.getVideoQuality()).setVideoCodecs(mInputParam.getVideoCodec()).setAction(CropKey.ACTION_SELECT_TIME).build();
